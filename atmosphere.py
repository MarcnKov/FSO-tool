...
class atmos(object):
    ...
    def moveScrns(self):
        ...
        """
        logger.info("Moving phase screens") 
        # If random screens are required:
        if self.config.randomScrns:
            logger.info("Randomly moving screens")
        ...
        for i in self.wholeScrns:
            ...
            if (self.scrnPos[i][0] + self.scrn_size) >= self.wholeScrnSize:
                logger.info("pos > scrn_size: rolling phase screen X")
            ...
            if self.scrnPos[i][0] < 0:
                logger.info("pos < 0: rolling phase screen X")
            ...
            if (self.scrnPos[i][1] + self.scrn_size) >= self.wholeScrnSize:
                logger.info("pos > scrn_size: rolling Phase Screen Y")
                self.wholeScrns[i] = numpy.roll(self.wholeScrns[i],
            ...
            if self.scrnPos[i][1] < 0:
                logger.info("pos < 0: rolling Phase Screen Y")
